...
export class ReplayProvider extends EventEmitter {
  constructor(
    ...
    private fastForwardSeconds: number = 0
  ) {
    super();
  ...
  async run(): Promise<void> {
    ...
    const originalStartDate = new Date(
      initialStateJson.R.SessionInfo.StartDate
    );
    const originalEndDate = new Date(initialStateJson.R.SessionInfo.EndDate);

    const now = new Date();
    const adjustedStartDate = new Date(
      now.getTime() - this.fastForwardSeconds * 1000
    );
    ...
    if (initialStateJson.R.SessionData.Series) {
      initialStateJson.R.SessionData.Series.forEach((part: any) => {
        part.Utc = adjustedStartDate.toISOString();
      });
    }
